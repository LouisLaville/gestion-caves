...
int DataBaseManager::creerTablesSQLite() {
    ...
    const char* createVinTable = "CREATE TABLE Vin (idVin INTEGER PRIMARY KEY, region TEXT, cru TEXT, nom TEXT, millesime INTEGER, stock INTEGER, type TEXT);";
    ...
    const char* createCaveFournisseurTable = "CREATE TABLE CaveFournisseur (idCave INTEGER NOT NULL, idFournisseur INTEGER NOT NULL, PRIMARY KEY (idCave, idFournisseur), FOREIGN KEY (idCave) REFERENCES Cave (idCave), FOREIGN KEY (idFournisseur) REFERENCES Fournisseur (idFournisseur));";
    rc = sqlite3_exec(db, createCaveFournisseurTable, nullptr, nullptr, nullptr);
    if (rc) {
        std::cerr << "Erreur lors de la création de la table CaveFournisseur" << std::endl;
    }

    const char* createProposeTable = "CREATE TABLE Propose (idCave INTEGER NOT NULL, idVin INTEGER NOT NULL, prixBase DECIMAL(10,2), marge DECIMAL(5,2), PRIMARY KEY (idCave, idVin), FOREIGN KEY (idCave) REFERENCES Cave (idCave), FOREIGN KEY (idVin) REFERENCES Vin (idVin));";
    ...
    const char* createGereTable = "CREATE TABLE Gere (idVin INTEGER NOT NULL, dureeMinGarde INTEGER, anneeConsommation INTEGER, PRIMARY KEY (idVin), FOREIGN KEY (idVin) REFERENCES Vin (idVin));";
